        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-6" >
                        <p class="d-flex justify-content-start"><strong>Capster:</strong> {{ $transaction->capster->name }}</p>
                    </div>
                </div>
                <h5 class="text-center mb-3">### PAID ###</h5>
                @foreach($transaction_details as $transaction_detail)
                    <div class="d-flex justify-content-between">
                        <strong>{{ $transaction_detail->service->name }} </strong>
                    </div>
                    <div class="d-flex justify-content-between">
                        <span>{{ $transaction_detail->service->price }}  x {{ $transaction_detail->quantity }}</span>
                        <span>{{ $transaction_detail->service->price * $transaction_detail->quantity }}</span>
                    </div>
                @endforeach
                <div class="d-flex justify-content-between mt-2">
                    <span>Kembali</span>
                    <span>{{ $transaction->money_received - $transaction->total }}</span>
                </div>
                <div class="d-flex justify-content-between mt-2">
                 <a href="{{ route('capster.export-pdf', $transaction->id) }}" type="button" class="btn btn-success col-12 print-btn">Print</a>
                </div>
            </div>
