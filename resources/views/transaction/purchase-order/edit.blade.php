...
<div class="card">
    <form action="{{ route('purchase-orders.update', $purchaseOrder->id) }}" method="POST"
        ...
        <h5 class="card-header">Edit Purchase Order</h5>
        <div class="card-body">
            <div class="form-body">
                <div class="form-group row @error('supplier_id') has-error @enderror">
                    ...
                    <div class="col-md-3">
                        <select name="supplier_id" id="supplier_id"
                            class="form-control supplier_id @error('supplier_id') has-error @enderror">
                ...
                <div class="form-group row @error('delivery_date') has-error @enderror">
                    ...
                    <div class="col-md-5">
                        <input type="text" name="delivery_date"
                            class="form-control delivery_date @error('delivery_date') has-error @enderror" autocomplete="delivery_date"
                            value="{{ old('delivery_date', $purchaseOrder->delivery_date) }}" readonly>
                ...
                <div class="form-group row @error('shipping_address') has-error @enderror">
                    ...
                    <div class="col-md-5">
                        <textarea name="shipping_address" id="purchaseOrderShippingAddress" class="form-control @error('shipping_address') is-invalid @enderror" style="height: 10em;">{{ old('shipping_address', $purchaseOrder->shipping_address) }}</textarea>
                            ...
                            <table class="table table-bordered table-striped table-hover purchase_order_table">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($purchaseOrderItems as $key => $value)
                                        <tr>
                                            <td>
                                                <select name="purchaseOrderInput[{{ $value['id'] }}][product_id]" id="product_id{{ $value['id'] }}"
                                                    class="form-control product_id choose-product-id">
                                                    <option value="">-- Choose Product --</option>
                                                    @foreach ($products as $product)
                                                        <option value="{{ $product->id }}"
                                                            {{ $value->product_id == $product->id ? 'selected' : '' }}>
                                                            {{ $product->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <td>
                                                <input type="text" class="form-control quantity" size="5" name="purchaseOrderInput[{{ $value['id'] }}][qty]" value="{{ $value['qty'] }}">
                                            </td>
                                            <td>
                                                <input type="number" step="any" class="form-control price" size="5" name="purchaseOrderInput[{{ $value['id'] }}][price]" value="{{ $value['price'] }}">
                                            </td>
                                            <td>
                                                <input type="number" step="any" class="form-control total_amount" size="7" name="purchaseOrderInput[{{ $value['id'] }}][total_amount]" value="{{ $value['total_amount'] }}">
                                            </td>
                                            <td>
                                        ...
                                        @php $sumTotal += $value['total_amount'] @endphp
                                    @endforeach
                                </tbody>
                            </table>
                            <hr>
