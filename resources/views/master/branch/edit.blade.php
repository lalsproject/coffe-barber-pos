...
@section('content')
    <div class="container">
        <div class="card">
            <form action="{{ route('branch.update', $branch->id) }}" class="form-horizontal needs-validation" method="POST">
                @csrf
                @method('PATCH')
                            ...
                            <div class="col-md-5">
                                <select name="status" id="status"
                                    ...
                                    <option value="active" {{ old('status') == 'active' ? 'selected' : '' }}
                                        {{ $branch->status == 'active' ? 'selected' : '' }}>Aktif</option>
                                    <option value="inactive" {{ old('status') == 'inactive' ? 'selected' : '' }}
                                        {{ $branch->status == 'inactive' ? 'selected' : '' }}>Tidak Aktif</option>
                                </select>
                                        ...
                                        <div class="row">
                                            <div class="offset-sm-3 col-md-5">
                                                ...
                                                <a href="{{ route('branch.index') }}"
                                                    class="btn btn-info btn-rounded text-white"> <i
