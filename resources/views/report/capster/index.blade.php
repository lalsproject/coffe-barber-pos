...
@section('title', 'Daily Revenue Report Capster')

@section('content')
@include('layouts.admin.include.message')
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h1 class="mb-0 mx-auto" style="font-weight: 600;">Daily Revenue Report Capster</h1>
    </div>
    <div class="card-body">
        <div class="card-body">
            {{-- Filter Date --}}
            <div class="row">
                    ...
                    <form class="form-inline mb-4" action="{{ route('capster.report.search') }}" method="GET">
                        ...
                        <div class="input-group mb-2 mr-sm-2">
                            <input type="text" name="start_date" id="start_date" class="form-control start_date
                                @error('start_date') is-invalid @enderror" value="{{ old('start_date', isset($start_date) ? $start_date : date('Y-m-d')) }}" readonly>
                        </div>
        
                        <label class="sr-only" for="start_date">End Date</label>
                        <div class="input-group mb-2 mr-sm-2">
                            <input type="text" name="end_date" id="end_date" class="form-control end_date
                                @error('end_date') is-invalid @enderror" value="{{ old('end_date', isset($end_date) ? $end_date : date('Y-m-d')) }}" readonly>
                        </div>
                        ...
                        <button type="submit" name="action" class="btn btn-success mb-2 ml-1" style="color: #fff; font-weight: bold;" value="export-pdf">
                            <i class="la la-file-pdf-o" style="color: #fff; font-size: 1.3em;"></i> Export PDF
                        </button>
                    </form>
                </div>
                <div class="col-md-1">
                    <form action="{{ route('capster.report.today') }}" method="GET">
                        <button type="submit" class="btn btn-primary mb-2" style="color: #fff;">
                ...
                <div class="col-md-1 ml-2">
                    <form action="{{ route('capster.report.weekly') }}" method="GET">
                        <button type="submit" class="btn btn-primary mb-2" style="color: #fff;">
                ...
                <div class="col-md-1 ml-2">
                    <form action="{{ route('capster.report.monthly',) }}" method="GET">
                        <button type="submit" class="btn btn-primary mb-2" style="color: #fff;">
        ...
        <div class="table-responsive">
            <table id="bs4-table" class="table table-striped table-bordered" style="width:100%">
                <thead>
                    <tr>
                        <th class="header-color text-white" style="width: 10px;">No</th>
                        <th class="header-color text-white">Capster Name</th>
                        <th class="header-color text-white">Service Name</th>
                        <th class="header-color text-white">Payment Method</th>
                        <th class="header-color text-white">Transaction Date</th>
                        <th class="header-color text-white">Status</th>
                        <th class="header-color text-white">Total Transaction</th>
                        <th class="header-color text-white"></th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($transactions as $transaction)
                        <tr>
                            ...
                            <td>
                                <span style="font-size: 1em; font-weight: bold;" class="badge badge-{{ $transaction->status == 'paid' ? 'success' : 'danger' }}">
                            ...
                            <td> 
                             <button onclick="onShowDialog({{ $transaction->id }})" type="submit" class="btn btn-primary mb-2" style="color: #fff;">
